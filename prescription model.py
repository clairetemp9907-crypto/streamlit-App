...
def load_data_and_train_model():
    # 1. 국내 공공데이터 로드 (웹 배포 시 경로 문제 방지를 위해 로컬 상대경로 설정)
    # ⚠️ 깃허브 리포지토리에 이 CSV 파일도 반드시 같은 경로에 올려주셔야 합니다.
    try:
        public_data = pd.read_csv('한국의약품안전관리원_용량주의약물_20240501.csv', encoding='cp949')
    except FileNotFoundError:
        # 파일이 없을 때를 대비한 가상의 예외 데이터 (테스트용)
        data_placeholder = {
    ...
    # 2. 캐글 약물 분류 기준 데이터 임의 학습 (Streamlit 내 무중단 작동용 임시 모델 학습)
    # 실제 ML 구동 파일이 분리되어 있다면 해당 피클 파일을 읽어오는 구조로 대체 가능합니다.
    # 여기서는 데모 웹의 독립적 작동을 위해 최소한의 결정트리 로직을 가상 학습시킵니다.
    data_demo = {
...
if selected_substance:
    ...
    if not matching_row.empty:
        ...
        if is_pediatric:
            ...
            filter_reason = "만 12세 미만 소아 환자의 불완전한 신장 배설 능력을 고려하여 성인 표준량 대비 50%를 자동으로 감량 적용하였습니다."
        else:
